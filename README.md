# Grocer App - JavaFX Desktop Application
...
Project3/
├── sql/
│   └── grocer_database.sql     # Database schema and initial data
├── src/
│   └── com/greengrocer/
...
```bash
   # MySQL
   mysql -u myuser -p1234 < sql/grocer_database.sql
   
   # Or MariaDB
   mariadb -u myuser -p1234 < sql/grocer_database.sql
   ```

   # Run as root user (the SQL file creates 'myuser' automatically)
   mysql -u root -p < sql/grocer_database.sql
   ```

   **Option 2: MySQL Workbench / phpMyAdmin**
   1. Connect as root user
   2. Open and execute `sql/grocer_database.sql`
